...
public class ver_se_existe
{
    ...
    public static int ver_email_dentro(TextBox txtEmail, string nome)
    {
        //ver se uma determinada matricula já existe na base de dados
        // TODO: Add constructor logic here
        //
        ...
        string constring = ConfigurationManager.ConnectionStrings["ConnectionString_usr"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        SqlCommand cmd = new SqlCommand("Select Email from Utilizador where Email= @EmailId AND ([Nome] NOT LIKE '%' + @Nome + '%')", con);
        cmd.Parameters.AddWithValue("@EmailId", txtEmail.Text);
    ...
    public static int ver_contribuinte_dentro(TextBox txt, string nome)
    {
        //ver se uma determinada matricula já existe na base de dados
        // TODO: Add constructor logic here
        //
        ...
        string constring = ConfigurationManager.ConnectionStrings["ConnectionString_usr"].ConnectionString;
        SqlConnection con = new SqlConnection(constring);
        SqlCommand cmd = new SqlCommand("Select N_contribuinte from Utilizador where N_contribuinte= @Id AND ([Nome] NOT LIKE '%' + @Nome + '%')", con);
